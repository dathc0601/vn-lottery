...
@section('title', isset($dayLabel) && $dayLabel ? 'Xổ Số Miền Nam ' . $dayLabel . ' - XSMN ' . $dayLabel . ' - Hàng Tuần' : ($isSpecificDate ? 'Kết quả xổ số Miền Nam ngày ' . $date->format('d/m/Y') . ' - XSMN' : 'XSMN - Kết Quả Xổ Số Miền Nam - SXMN Hôm Nay'))
...
@section('page-content')
    <h1 class="text-xl font-bold text-gray-800 mb-4">
        {{ isset($dayLabel) && $dayLabel ? 'Xổ Số Miền Nam ' . $dayLabel . ' - Hàng Tuần' : ($isSpecificDate ? 'Kết Quả Xổ Số Miền Nam ngày ' . $date->format('d/m/Y') : 'XSMN - Kết Quả Xổ Số Miền Nam Hôm Nay') }}
    </h1>
    ...
    @if(count($groupedResults) > 0)
        ...
        <div id="results-container">
            @include('partials.xsmn-grouped-results-list', ['groupedResults' => $groupedResults, 'region' => 'xsmn'])
        </div>
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (dateInput && dateInput.value) {
                ...
                window.location.href = `{{ route('xsmn.date', ['date' => '__DATE__']) }}`.replace('__DATE__', formattedDate);
            } else {
