@php
    ...
    $defaultProvinceText = $isXSMT ? 'các tỉnh Miền Trung' : 'các tỉnh Miền Nam';
    $provinceListText = $provinceCount > 0 ? implode(', ', $provinceNames) : $defaultProvinceText;
    $daiCount = $isXSMT ? '3' : '3-4';

    $faqs = [
        [
            'question' => "Soi cầu {$regionCode} là gì?",
            'answer' => "Soi cầu {$regionCode} là phương pháp phân tích kết quả xổ số {$regionFullName} dựa trên các thuật toán thống kê để dự đoán các con số có khả năng xuất hiện cao trong các kỳ quay tiếp theo. {$regionCode} có nhiều đài quay cùng một ngày, ví dụ hôm nay có: {$provinceListText}."
        ],
        [
            'question' => "{$regionCode} có bao nhiêu đài mỗi ngày?",
            'answer' => "Xổ số {$regionFullName} có từ {$daiCount} đài quay mỗi ngày, tùy thuộc vào thứ trong tuần. Mỗi đài có kết quả độc lập và có thể áp dụng các phương pháp soi cầu riêng cho từng đài."
        ],
        [
            'question' => "Làm thế nào để sử dụng dự đoán {$regionCode}?",
            'answer' => "Bạn có thể tham khảo các số dự đoán được cung cấp trên trang này, bao gồm: dự đoán tổng hợp cho tất cả các đài (bao lô {$provinceCount} đài, xiên 2, 3 càng đẹp) và dự đoán riêng cho từng tỉnh (giải tám, đầu đuôi, bao lô 2 số)."
        ],
        [
            'question' => "Dự đoán {$regionCode} được cập nhật khi nào?",
            'answer' => "Dự đoán {$regionCode} được tự động cập nhật hàng ngày vào lúc 2h sáng, dựa trên kết quả xổ số ngày hôm trước của tất cả các đài."
        ],
        [
...
<div class="my-6">
    <h2 class="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
        ...
        Câu hỏi thường gặp về {{ $regionCode }}
    </h2>
