...
@section('title', 'XSMN Trực Tiếp - Kết Quả Xổ Số Miền Nam Hôm Nay')
...
<div x-data="liveXsmn()" x-init="init()">
    <!-- Two-Column Layout -->
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="bg-white rounded shadow overflow-hidden mb-4">
                <div class="bg-[#ff6600] text-white px-4 py-2">
                    <h1 class="text-lg font-bold">XSMN Trực Tiếp - Kết Quả Xổ Số Miền Nam Hôm Nay</h1>
                </div>
            </div>
            ...
            <div class="mb-4">
                <template x-if="status === 'before'">
                    <div class="bg-blue-50 border border-blue-200 px-4 py-3 rounded flex items-center">
                        ...
                        <span class="text-sm text-blue-800">Phiên quay số bắt đầu lúc <strong>16:15</strong>. Kết quả sẽ được cập nhật tự động.</span>
                    </div>
                </template>
            ...
            <template x-if="isPreviousDay && todayDateDisplay && resultsDateDisplay">
                <div class="bg-orange-50 border border-orange-300 px-4 py-3 rounded mb-4 text-sm text-gray-800">
                    KQXS Miền Nam hôm nay <span x-text="todayDateDisplay"></span> hiện <strong class="text-red-600">chưa có</strong>, dưới đây là kết quả XSMN mới nhất (<span x-text="resultsDateDisplay"></span>)
                </div>
            </template>
            ...
            <div class="result-card border border-gray-300 bg-white mb-5">
                ...
                <div class="bg-[#fff8dc] px-4 py-3 border-b border-gray-300">
                    <h2 class="text-lg font-semibold text-center text-gray-800">
                        XSMN Trực Tiếp - <span x-text="todayFormatted"></span>
                    </h2>
                    <div class="text-center text-sm text-[#0066cc] mt-1">
                        <a href="{{ route('xsmn') }}" class="hover:underline">XSMN</a> /
                        <span>Trực tiếp</span>
                    </div>
                </div>
                        ...
                        <table class="w-full border-collapse">
                            <thead>
                                <tr class="bg-gray-100">
                                    ...
                                    <th class="border border-gray-300 py-2 px-3 text-center text-[#333399]" :colspan="provinces.length">Lô tô XSMN trực tiếp</th>
                                </tr>
                                <tr class="bg-gray-100">
            ...
            <div class="sidebar-section mt-4">
                <div class="sidebar-header">Thông tin trực tiếp XSMN</div>
                <div class="p-4 text-sm space-y-2 text-gray-700">
                    <p>Kết quả xổ số miền Nam được mở thưởng trực tiếp lúc <strong>16:15</strong> hàng ngày.</p>
                    <p>Mỗi ngày có 2-4 tỉnh quay số đồng thời, kết quả được cập nhật tự động. Bạn không cần tải lại trang.</p>
...
function liveXsmn() {
    return {
        ...
        async fetchResults() {
            try {
                const response = await fetch('{{ route("api.xsmn.live") }}');
                ...
                console.error('Failed to fetch live XSMN results:', e);
            }
        },
        ...
        scheduleNextPoll() {
            ...
            // 16:15 = 975, 16:50 = 1010
            const liveStart = 16 * 60 + 15;
            const liveEnd = 16 * 60 + 50;
